...
const MONGODB_URI =
	...
	"mongodb+srv://aahar_admin:SOvbq4SBysa1cCq3@cluster0.hshafav.mongodb.net/?appName=Cluster0";
...
const seedData = async () => {
	...
	const chef2 = await Chef.create({
		name: "Chef Suresh",
		specialty: "Curries & Rice",
		avatar: "🧑‍🍳",
	});
	const chef3 = await Chef.create({
		name: "Chef Anita",
		specialty: "Snacks & Refreshments",
		avatar: "👩‍🍳",
	});
	...
	const fullMenu = [
		...
		},
		{
			...
			items: [
				...
				"Jeera Rice",
				"Chapati",
				"Fried Mirchi Lemon Garlic Pickle",
				"Rice Kheer",
				"Chicken Sukka",
			],
		},
		...
		},
		{
			day: "Tuesday",
			meal: "Snacks",
			items: ["Samosa Chaat", "Lemonade", "Tea + Coffee + Milk"],
		},
		{
			day: "Tuesday",
			meal: "Dinner",
			items: [
				"Onion Tomato Salad",
				"Veg Raita / Rajasthani Dal",
				"Bhaji+Fried Curd",
				"Masala Pulao / Jeera Rice",
				"Pav / Fried Baati",
				"Lahsun Chutney",
				"Suji Halwa / Churma",
				"Fish Curry + Rice",
		...
		// === WEDNESDAY ===
		{
			...
			meal: "Breakfast",
			items: [
				"Tomato Onion Coriander Uttapam",
		...
		},
		{
			...
			meal: "Snacks",
			items: [
				"Coleslaw sandwich/peri peri sandwich",
				"Banana Milkshake",
				"Elaichi Tea",
			],
		},
		...
		// === THURSDAY ===
		{
			...
			meal: "Breakfast",
			items: [
				"Aloo Onion Paratha/Methi/Garlic Paratha",
				"Curd",
				"Lahsun Chutney",
				"Egg Bhurjee",
				"White Bread+Butter+Jam",
		...
		},
		{
			...
			items: [
				"Bombay bhel",
				"Green & Imli chutney",
				"Apple crush/mint crush",
				"Elaichi Tea + Coffee + Milk",
			],
		},
		{
			day: "Friday",
		...
		},
		{
			...
			items: [
				...
				"Dry Fruit Lassi",
				"Chapati",
				"Green Chutney Mirchi Lemon Mix Veg Pickle",
				"Fryums",
			],
		},
		{
		...
		},
		{
			...
			items: [
				...
				"Chapati",
				"Lemon Garlic Pickle",
				"Chocolate Ice cream/Fruit Custard",
				"Chicken Angara",
			],
		},
		...
		// === SUNDAY ===
		{
			...
			meal: "Breakfast",
			items: [
				"Schezwan Butter Dosa/ Onion Dosa",
				"Sambar+Peanut Chutney",
				"Ginger chutney",
				"-",
				"Boiled Eggs + Masala",
		...
		},
		{
			...
			items: [
				...
				"Plain Rice",
				"Chapati",
				"Fried Mirchi + Lemon + Mix Veg Pickle",
				"Manchurian",
				"Hyderabadi Chicken Dum Biryani",
			],
		},
	...
	console.log(
		`Database seeded with ${dishDocs.length} items across the entire week!`,
	);
};
...
app.get("/api/menu/today", async (req, res) => {
	...
	const allDishes = await Dish.find({ dayOfWeek: currentDay }).populate(
		"chefId",
	);
...
app.post("/api/like/:dishId", async (req, res) => {
	...
	if (currentActiveMeal !== dishMealType) {
		return res
			.status(403)
			.json({ error: `Voting for ${dishMealType} is currently closed.` });
	}
	...
	if (existingVote) {
		return res.status(403).json({
			error: `You have already voted for a ${dishMealType} item today.`,
		});
	}
...
app.put("/api/admin/dishes/:id", async (req, res) => {
	...
	const updatedDish = await Dish.findByIdAndUpdate(
		req.params.id,
		{ name, chefId, mealType, dayOfWeek },
		{ new: true },
	);
